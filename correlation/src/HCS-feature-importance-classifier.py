...
def collate(dictionary):
    ...
    return pd.concat([addcategory(preprocess(dataframe).sample(minrows), ID) for (ID, dataframe) in enumerate(dictionary.values())], ignore_index=True)
...
baseline(features, label, model)
featureimportance(features, label, names, model, 'PI')
featureimportance(features, label, names, model, 'MDI')
